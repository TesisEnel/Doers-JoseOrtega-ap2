...
fun RecompensasOverview(
    ...
    BoxWithConstraints(modifier = modifier.fillMaxSize()) {
        ...
        Column(
                ...
                AnimatedVisibility(visible = !isHorizontal) {
                    Row(
                        ...
                        RewardCounter(
                            ...
                            recompensas.count { it.estado == EstadoRecompensa.COMPLETADA },
                            EstadoRecompensa.COMPLETADA
                        )
                        RewardCounter(
                            "Recompensas disponibles",
                            recompensas.count { it.estado == EstadoRecompensa.PENDIENTE },
            ...
            Text(
                text = if (recompensas.isEmpty()) "No se han creado recompensas" else "Listado de pendientes",
                fontSize = 22.sp,
                fontWeight = FontWeight.Bold,
                ...
                items(recompensas) { recompensa ->
                    RecompensaCard(
                        ...
                        onCondicionChange = { nuevaCondicion ->
                            viewModel.onCondicionChange(recompensa.recompensaId, nuevaCondicion)
                        }
                    )
                }
            }
